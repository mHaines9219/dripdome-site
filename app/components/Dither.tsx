...
void main() {
  ...
  // Normalize/clamp so the background isn't "mostly black" with only a thin visible band.
  f = clamp(f, 0.0, 1.0);
  f = smoothstep(0.0, 0.9, f);
  // Give the whole scene a base tint so it reads as an actual background behind content.
  vec3 col = waveColor * (0.18 + 0.82 * f);
...
vec3 dither(vec2 uv, vec3 color) {
  ...
  // Lower bias so we don't crush everything toward black.
  float bias = 0.05;
